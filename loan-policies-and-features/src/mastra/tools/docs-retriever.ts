...
export const docsRetriever = createTool({
  id: "docsRetriever",
  description: "Search knowledge/<namespace> for relevant loan policy and feature information in .md|.mdx|.txt files.",
  ...
  execute: async (raw: any) => {
    ...
    const namespace = (base?.namespace ?? "loans").toString().trim();
    ...
    if (!(await dirExists(root))) {
      return { results: [], error: `Namespace '${namespace}' not found at ${root}.` };
    }
    ...
    try {
      ...
      return { results: [], error: `Failed to read namespace directory: ${e?.message || "unknown error"}` };
    }

    const files = entries.filter(f => /\.(md|mdx|txt)$/i.test(f));
    if (files.length === 0) {
      return { results: [], error: `No .md|.mdx|.txt files in '${namespace}'.` };
    }

    const { phrases, words } = extractTokens(query);
    const tokens = [...phrases, ...words];
    if (tokens.length === 0) {
      return { results: [], sources: [], info: "No searchable tokens in query." };
    }
    ...
    if (allHits.length === 0) {
      return { results: [], sources: [], info: "No matches found." };
    }
