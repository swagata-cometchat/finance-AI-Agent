...
// Force real Node FS (avoid bundler polyfills)
...
async function resolveLocalPathSmart(p: string): Promise<{ abs: string; tried: string[] }> {
  ...
  console.log("[resolveLocalPathSmart] p=", p, "\n repoRoot=", repoRoot, "\n bases=", bases);
  ...
  console.log("[resolveLocalPathSmart] ENOENT p=", p, "\n tried=", tried);
  return { abs: "", tried };
}
...
function buildAxiosCfg(allowInsecureTLS: boolean): AxiosRequestConfig {
  ...
  return {
    ...
    headers: {
      "User-Agent": "Mozilla/5.0 (LoanPoliciesAgent/1.0; +https://example.test)",
      "Accept-Language": "en",
    },
  };
}
...
async function loadPdfParse(): Promise<(buf: Buffer) => Promise<{ text: string }>> {
  try {
    // Avoid package root side-effects by preferring the internal module
    const mod: any = await import("pdf-parse/lib/pdf-parse.js");
    return (mod.default || mod) as any;
  } catch {
    const mod: any = await import("pdf-parse"); // fallback
    return (mod.default || mod) as any;
  }
}
...
async function ingestLocalFile(filePath: string, root: string) {
  ...
  if (!abs) {
    throw Object.assign(new Error(`local-file-not-found: ${filePath}`), {
      code: "ENOENT",
      filePath,
      tried,
      cwd: process.cwd(),
  ...
  console.log("[ingestLocalFile] reading:", abs);

  // Probe OS-level access first
  try {
    const fh = await fs.open(abs, "r");
    await fh.close();
    console.log("[ingestLocalFile] open() OK:", abs);
  } catch (e: any) {
    console.error("[ingestLocalFile] open() FAILED:", abs, e);
    throw Object.assign(new Error("open-failed"), {
      code: e?.code || "OPEN_FAIL",
      filePath: abs,
      tried,
      cwd: process.cwd(),
      here: HERE,
  ...
  try {
    if (/\.pdf$/i.test(abs)) {
      const buf = await readFileStream(abs);    // streaming for robustness
...
export const ingestSources = createTool({
  id: "ingestSources",
  description: "Download and normalize loan policy sources (URLs, local files, or raw text) into the knowledge/loans directory.",
  inputSchema: z.object({
    sources: z.array(z.string()).min(0).default([]),
    files: z.array(z.string()).min(0).default([]),
    namespace: z.string().default("loans"),
  ...
  execute: async ({ context }) => {
    ...
    console.log("INGEST ctx ->", {
      sources, files, namespace, allowInsecureTLS,
      serverCwd: process.cwd()
    });

    const repoRoot = await getRepoRoot();
    const root = path.resolve(repoRoot, "knowledge", namespace); // anchor writes to real repo root
      ...
      try {
        if (/^https?:\/\//i.test(s)) {
          ...
          if (abs) {
            ...
            written.push({ file: `ERROR: local-file-not-found @ ${s}`, bytes: 0, kind: "file", source: s });
          } else {
            const digest = Buffer.from(s).toString("base64").slice(0, 16);
            const fname = `text-${digest}.md`;
            const fpath = path.join(root, fname);
            const content = `# Pasted Text\n\n${s}`;
            await fs.writeFile(fpath, content, "utf8");
